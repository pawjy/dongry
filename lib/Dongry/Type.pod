...
Accept a reference to a byte string or an C<undef> value.  The value
is dereferenced and treated as a byte string when used as part of SQL.
The C<undef> value represents the C<NULL> value in SQL.
...
Accept a character string or an C<undef> value.  The value is encoded
in the Perl C<utf-8> encoding when used as part of SQL.  The C<undef>
value represents the C<NULL> value in SQL.
...
Accept a reference to a character string or an C<undef> value.  The
value is dereferenced and encoded in the Perl C<utf-8> encoding when
used as part of SQL.  The C<undef> value represents the C<NULL> value
in SQL.
...
Accept a byte string or an C<undef> value.  The value is treated as a
byte string when used as part of SQL.  Trailing U+0000 NULL characters
in the string from the database is dropped to convert into the Perl
representation.  The C<undef> value represents the C<NULL> value in
SQL.
...
Accept a character string or an C<undef> value.  The value is encoded
in the Perl C<utf-8> encoding when used as part of SQL.  Trailing
U+0000 NULL characters in the string from the database is dropped to
convert into the Perl representation.  The C<undef> value represents
the C<NULL> value in SQL.
