...
but they are schema-aware in that values specified in the "where"
...
The method returns a (possibly empty) L<List::Rubyish> object
containing table row objects representing the selected rows.

Following options for the C<select> method of the database object also
apply to this method: C<fields>, C<group>, C<order>, C<offset>,
C<limit>, C<source_name>, and C<lock>.
...
C<multiple> option is specified, a L<List::Rubyish> object containing
the table row objects is specified as the first argument to the
method.  Otherwise, a table row object, if any, or C<undef> otherwise,
is specified as the argument to the method.
