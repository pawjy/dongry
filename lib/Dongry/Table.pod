...
database object for more information.
...
of them.  Both set of values would contain any default values.  The
method does not return a value in asynchronous mode.
...
The method does not support the C<cb> option and asynchronous mode.
...
returned.  Otherwise, the C<undef> value is returned.  However, the
method does not return a value in asynchronous mode.
...
objects representing the selected rows.  However, the method does not
