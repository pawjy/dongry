...
Following function names are available: C<-count> (in SQL,
C<COUNT()>), C<-min> (in SQL, C<MIN()>), C<-max> (in SQL, C<MAX()>),
C<-sum> (in SQL, C<SUM()>), C<-date> (in SQL, C<DATE()>), C<-x> (in
SQL, C<X()>), and C<-y> (in SQL, C<Y()>).

In addition, following name-value pairs can be specified in the hash
reference: C<< as => name >> names an alias for the function value (in
SQL, C<function() AS name>), and C<< distinct => boolean >> adds the
C<DISTINCT> keyword (in SQL, C<function(DISTINCT args)>).  For the
C<-date> function, C<< delta => seconds >> adds the specified numbers
of seconds to the argument (in SQL, C<DATE(args + INTERVAL delta
SECOND)>).

The C<-distance> function can be used to obtain geographical distance
between the location represented by a C<GEOMETRY> column and the
specified location.  It is a shorthand of SQL function C<<
GLength(GeomFromText(CONCAT('LineString(lon lat,', X(column), ' ',
Y(column),')'))) >>, where I<lat> and I<lon> are values for keys
C<lat> and C<lon> and I<column> is the value for the key C<-distance>.

Instead of the function name, keyword C<-column> can be used to denote
the column whoes name is specified as the value.  For the C<-column>
"function", only the C<as> option is available.
...
hash reference whoes key is C<-eq> and whoes value is the specified
value.
...
pairs.  It represents the SQL statement whoes placeholders are
...
Replaced by the value of the key/value pair whoes key is equal to
...
value for the column whoes name is equal to I<key>.
...
If the table schema is available, the value is interpreted as the
value for the column whoes name is equal to I<column>.
...
Replaced by the value of the key/value pair whoes key is equal to
I<key>.  It is interpreted as an SQL expression.  The value must be a
"where" construct or an C<undef> value.  If the value is C<undef>, SQL
expression C<1 = 1>, which is always true, is inserted instead.
