...
Methods of the transaction object is as follows:
...
If you don't want a transaction session but you'd like to force
subsequent operations done using a specific data source, the "force
source" feature can be used:

=over 4

=item $force = $db->force_source_name ($source_name)

Return a "guard" object that forces use of a data source specified as
the argument.

While the I<$force> object is in scope (or before the C<end> method is
invoked), the C<execute> method and the other methods to execute SQL
statements act as if the C<source_name> option with the specified
source name is specified, except that if the C<source_name> is
explicitly specified to use another data source, the execution would
result in an error.
...
The "force" object has a method:

=over 4

=item $force->end

End to force the data source.

Note that when the I<$force> object is destroyed this method is
...
statement to execute, which possibly contains placeholders.  The
second argument is an array reference, which contains values bound to
the placeholders.  The second argument can be omitted (i.e. can be
