...
the asynchronous event-based model by using the L<AnyEvent::DBI>
module as its backend.  Since both modes can be switched or combined
easily, you can transit one from the other, or you can partially
change the processing mode, in very low cost.
...
Whether the asynchronous mode using L<AnyEvent::DBI> should be enabled
or not.
...
C<onconnect> handler is invoked after the L<AnyEvent::DBI> object is
instantiated, but the actual connection might not been established at
that time.  You can assume that the C<onconnect> handler is invoked
before any SQL statements or start of transactions are queued to
execute with the connection, even when the C<anyevent> option is true.
...
(L<AnyEvent::DBI>) modes different in several ways.
...
In the asynchronous mode of L<AnyEvent::DBI>, as SQL statements are
executed asynchronously, errors are also reported asynchronously.  If
the method to access to the database accepts the C<cb> (callback)
option, the callback function is invoked even if there is an error,
but with the argument describing the error (See the documentation of
the C<execute> method for details).  Then, the C<onerror> handler of
the database object is invoked for the error.  No exception is thrown
by Dongry, as it could terminate the AnyEvent event loop.  For this
reason, the C<onerror> handler and C<cb> functions should not throw
any exception in asynchronous mode.  Once an error is detected,
whether fatal or not, the L<AnyEvent::DBI> connection is disabled and
any subsequent requests would result in error.  There might be
subsequent SQL requests already queued but not executed yet, and it is
unclear whether they should be executed after the occurence of an
error.
...
L<AnyEvent::DBI> is used for the connection, the method only does
instantiate the L<AnyEvent::DBI> method and the actual connection is
...
In synchronous mode, the code reference is synchronously invoked when
the connection has been successfully established.

In asynchronos mode, the code reference is asynchronously invoked when
the connection has been successfully or erronerously established,
...
L<AnyEvent::DBI>) mode, but controlling how queued SQL statements are
executed and how errors are handled might be difficult than in
synchronous mode.  Since L<AnyEvent::DBI> preserves the order of SQL
statements and transaction start/end methods, you can write
transaction and SQL methods sequentially.  However, you have to use
the C<cb> (callback) function to observe whether the execution has
...
C<*_as_row> methods cannot be used on the result object.
...
In synchronous mode, the code reference is synchronously invoked when
the SQL has successfully been executed.

In asynchronos (or L<AnyEvent::DBI>) mode, the code reference is
asynchronously invoked when the SQL has been executed, successfully or
erroneously.
...
result object might be useful for this purpose.  The method does not
return a value in asynchronous mode.
...
C<first_as_row>.  The method does not return a value in asynchronous
mode.
...
Examples:
  ...
  $db->select ('foo', {user_id => $uid}, cb => sub {
    if ($_[1]->is_success) {
      print $_[1]->all->map (sub { $_->get ('name') })->join ("\t");
    }
    $cv->send;
  }, source_name => 'ae');
  $cv->recv;
...
generated by this method.  The method does not return a value in
asynchronous mode.
...
loads the C<DBIx::ShowSQL> module.
...
If you use asynchronous mode, the L<AnyEvent::DBI> module is required.
In addition, modules L<AnyEvent::DBI::Hashref> and
L<AnyEvent::DBI::Carp> available from
<https://github.com/wakaba/perl-rdb-utils/> is required.
...
L<AnyEvent::DBI>, L<AnyEvent::DBI::Carp>.
