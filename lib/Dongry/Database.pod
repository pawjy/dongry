...
Transactions are not supported in asynchronous (or
L<AnyEvent::MySQL::Client>) mode for now.  They are to be supported in
future revision.
...
Perl 5.10 or later is required.  Although the module can be used in
Perl 5.8, some tests fail due to bugs of perl.  In addition, a module
required by the asynchronous mode does not support Perl 5.8 so that
only the synchronous mode is available to Perl 5.8 applications.

For the synchronous mode, the L<DBI> module (and L<DBD::mysql> module
for connecting MySQL databases) is required.

For the asynchronous mode, the L<AnyEvent::MySQL::Client> module in
the <https://github.com/wakaba/perl-anyevent-mysql-client> repository
is required.
