...
object holds the execution result of the SQL statement.  It can be
accessible from following methods of the result object:
...
read operations).  Please note the semantics of the reported number
depends on the exact SQL statement and the implementation of the
database, and might not be always useful for the application.  For
example, the row count for the C<INSERT IGNORE> statement might not be
equal to the number of rows actually inserted into the table.
