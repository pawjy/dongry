...
returns a L<List::Rubyish> object which contains zero or more hash
references representing the rows.  The hash reference consists of
key/value pairs representing column names and their values.
...
Dongry requires the L<DBI> module as underlying database API and
L<List::Rubyish> as list object implementation.  Additional modules
might be used by L<Dongry::Type> modules, but they are not required as
long as you don't use such modules specificalliy.
