...
Error handling in synchronous (L<DBI> and L<DBD::mysql>) mode and
asynchronous (L<AnyEvent::MySQL::Client>) mode are different in
several ways.
...
For the synchronous mode, the modules L<DBI> and L<DBD::mysql> are
required.
