...
returned, but this could be modified by C<item_list_filter>.  However,
the method does not return a value in asynchronous mode.
...
C<item_list_filter>.  However, the method does not return a value in
asynchronous mode.
...
of rows matched.  However, the method does not return a value in
asynchronous mode.
